...
public class SubjectTeacherDAO extends BaseHibernateDAO {
    ...
    public SubjectTeacher findBySubjetTeacher(UserMaster teacher, Subject subject) {
        log.debug("getting SubjectTeacher instance with teacher : " + teacher.toString() + "And subject : " + subject.toString());
        try {
            String queryString = "from SubjectTeacher st where st.userMaster.userIdPk = " + teacher.getUserIdPk() + " and st.subject.subjectIdPk = " + subject.getSubjectIdPk();
            Query queryObject = getSession().createQuery(queryString);
            return (SubjectTeacher) queryObject.uniqueResult();
        } catch (RuntimeException re) {
            log.error("get failed", re);
            throw re;
        }
    ...
    public SubjectTeacher findSubjectTeacherForTeacher(UserMaster teacher) {
        log.debug("getting SubjectTeacher instance with teacher : " + teacher.toString());
        try {
            String queryString = "from SubjectTeacher st where st.userMaster.userIdPk = " + teacher.getUserIdPk();
            Query queryObject = getSession().createQuery(queryString);
            return (SubjectTeacher) queryObject.uniqueResult();
        } catch (RuntimeException re) {
            log.error("get failed", re);
